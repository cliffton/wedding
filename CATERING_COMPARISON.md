# Catering Quotes Comparison
**Event Date:** October 3, 2026 | **Guest Count:** 170

---

## Summary Pricing

| Metric | From Scratch (No Apps) | From Scratch (w/ All Apps) | Hometown Chef |
|--------|----------------------|--------------------------|---------------|
| **Base Total** | $11,035 | $12,480 | $15,346.39 |
| **Per Person Cost** | ~$64.91 | ~$73.41 | $90.27 |
| **Includes Tax & Gratuity?** | Yes (20%) | Yes (20%) | Partial (8.25% tax only, no gratuity) |
| **Appetizers (pcs/person)** | 0 | ~4 | 4 |

**Notes:** 
- From Scratch base includes 2 entrees, 2 sides, 1 salad
- HTC: Add 18-20% gratuity to true total (~$18,108-$18,415)
- From Scratch total includes tax & 20% gratuity upfront

---

## Core Package

| Item | From Scratch | Hometown Chef |
|------|--------------|---------------|
| **Base Package** | Elegance: $5,250 | Premium China Buffet: $6,800 (@ $40pp) |
| **What's Included** | 2 entrées, 2 sides, 1 salad, lemonade & water | Choice of 2 entrées, any salad, 2 sides, dinner rolls & butter |

---

## Appetizers

### Hometown Chef (Included)
| Item | Per Person | Total Cost |
|------|-----------|-----------|
| Birria Taco Shooter | 1 pp | Included in $13/pp |
| Short Rib Potato Vol Al Vent | 1.5 pp | Included in $13/pp |
| Spinach & Artichoke Stuffed Mushrooms (V) | 1.5 pp | Included in $13/pp |
| **Total per person** | 4 pieces | **$13.00/person = $2,210 total** |

### From Scratch (All Optional - NOT Included)
| Item | Price | Cost for 170 Guests |
|------|-------|-------------------|
| Vegan Lettuce Wraps | $25/dozen (10 per dozen) | $425 (170 ÷ 10 = 17 dozen) |
| Korean Beef Tacos | $40/dozen (10 per dozen) | $680 (170 ÷ 10 = 17 dozen) |
| Chicken Teriyaki Skewers | $120/pan (~60 pcs per pan) | $340 (170 ÷ 60 = 3 pans) |

**From Scratch Appetizer Scenarios:**
- **None:** $0 (save $2,210)
- **One option (e.g., Tacos):** $680 (save $1,530 vs HTC)
- **All three options:** $1,445 (save $765 vs HTC)
- **Match HTC quantity (4 pcs per person):** ~$1,200-$1,600 estimated

---

## Entrees

| Item | From Scratch | Hometown Chef |
|------|--------------|---------------|
| **Entrée Options** | 1 entrée (not specified) | Choose 2 from: Poblano Creamy Chicken OR Atlantic Cod Filet (GF) |
| **Cost** | Included in $5,250 | Included in $40pp price |

---

## Salad

| Item | From Scratch | Hometown Chef |
|------|--------------|---------------|
| **Salad** | 1 salad (not specified) | Chef's House Salad - Pre-Set (Mixed Greens, Baby Tomatoes, Dried Cranberries, Feta, Bacon, Garlic Croutons w/ Ranch or Balsamic) |
| **Cost** | Included | Included |

---

## Sides

| Item | From Scratch | Hometown Chef |
|------|--------------|---------------|
| **Side Options** | 2 sides (not specified) | Choose 2 from: Rice Pilaf (GF)(V) OR Penne with Creamy Tomato Basil (V) |
| **Cost** | Included in $5,250 | Included in $40pp price |

---

## Service & Staffing

| Item | From Scratch | Hometown Chef |
|------|--------------|---------------|
| **Chef Services** | 8 hrs @ $165/hr = $1,320 | Included in package |
| **Server Staff** | 15 staff @ $200 = $3,000 | Included in package |
| **Cambros (Equipment)** | 2 @ $170 = $340 | Included in package |
| **Seated Meal Service** | Not explicitly mentioned | $15.00/person = $2,550 (plated service) |
| **Cake Cutting** | $250 | Included (plates & forks provided) |
| **Bar Service** | 4 hrs minimum = $800 | Not included in main package |

---

## Beverages

| Item | From Scratch | Hometown Chef |
|------|--------------|---------------|
| **Beverages Included** | Lemonade & Water | Iced Tea Station (8 gallons @ $18 = $144) & Filtered Water Station ($35) |
| **Bar Service** | $800 (4 hrs minimum) | TBD - separately priced |

---

## Special Services

| Item | From Scratch | Hometown Chef |
|------|--------------|---------------|
| **Delivery Fee** | $75 | $75 |
| **Travel Fee** | Included in delivery | N/A |
| **Production Fee** | Included | 20% = $2,362.80 |

---

## Additional Costs & Fees

| Item | From Scratch | Hometown Chef |
|------|--------------|---------------|
| **Sales Tax** | Included in estimate | 8.25% = $1,169.59 |
| **Gratuity** | 20% included | NOT included; 15-20% optional at client discretion |
| **Credit Card Surcharge** | Not mentioned | 3% surcharge for credit cards |
| **Initial Deposit** | $250 Save-the-Date Deposit | $1,500 non-refundable retainer (50% bar only) |

---

## Kids Meals & Special Accommodations

| Item | From Scratch | Hometown Chef |
|------|--------------|---------------|
| **Kids Meals** | Not specified | Chicken Tenders & Mac & Cheese (Ages 3-10) - Half Price of Adult Meal (TBD) |
| **Vendor Meals** | Not mentioned | Available (TBD - specify which vendors) |
| **Dietary Restrictions** | Not specified | Vegan/Vegetarian options available (TBD) |

---

## Invoicing & Pricing Structure

| Item | From Scratch | Hometown Chef |
|------|--------------|---------------|
| **Food Subtotal** | N/A (lump sum package) | $11,814.00 |
| **Per Person Cost (Food)** | ~$30.77 | $69.49 |
| **Production Fee** | N/A | 20% = $2,362.80 |
| **Pre-Tax Subtotal** | ~$9,196 | $14,176.80 |
| **Tax Included** | Yes (in estimate) | 8.25% = $1,169.59 |
| **Final Balance** | $11,035 | $15,346.39 |

---

## Payment Terms

| Item | From Scratch | Hometown Chef |
|------|--------------|---------------|
| **Deposit** | $250 (applies toward reservation if booked within 48 hrs) | $1,500 non-refundable retainer due at signing |
| **Payment Schedule** | Final balance 3 weeks before event via structured payment plan | Equal monthly payments; minimum guest count due 2 weeks prior |
| **Cancellation Policy** | Not detailed | 75% of payments refundable up to 90 days prior (except $1,500 retainer) |

---

## Key Differences at a Glance

### From Scratch Advantages:
✅ Lower total cost ($11,035 vs $15,346.39)  
✅ Includes gratuity (20%)  
✅ Lower per-person cost ($64.91 vs $90.27)  
✅ Lower deposit ($250 vs $1,500)  

### Hometown Chef Advantages:
✅ **3 appetizers per person included** (vs optional for FS)  
✅ **Plated seated service included** ($15pp)  
✅ More detailed menu specifications  
✅ Multiple entree/side options for guests to choose  
✅ Better beverage setup (iced tea station + filtered water)  
✅ Pre-set salad is specified  
✅ Kids meals available  
✅ Dietary accommodations (vegan/vegetarian options listed)  
✅ More detailed contract terms  

---

## Cost Difference Breakdown

**Hometown Chef costs $4,311.39 more ($15,346.39 - $11,035)**

Main reasons:
1. **Production Fee (20%):** +$2,362.80
2. **Seated Meal Service:** +$2,550
3. **Appetizers included:** +$2,210
4. **Beverage Stations:** +$179
5. Offset by: Not including 20% gratuity upfront (discretionary)

---

## Questions to Ask Each Caterer

1. **From Scratch:**
   - What are the specific entrée, sides, and salad options?
   - Is bar service included or additional?
   - Does the estimate include all taxes and fees?

2. **Hometown Chef:**
   - What's the typical gratuity rate? (Needed for true cost comparison)
   - Can we adjust the menu or is it fixed?
   - Any upcharge for customization?

---

*Last Updated: April 20, 2026*
